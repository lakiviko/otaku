...
export async function getProxiedImage(pathParts, userAgent) {
  ...
  let upstream = await fetchImageByPath(imagePath, userAgent);
  ...
  if (!upstream.ok && upstream.status === 404 && requestedSize !== "original") {
    ...
    upstream = await fetchImageByPath(originalPath, userAgent);
  }
...
async function fetchImageByPath(imagePath, userAgent) {
  const sourceUrl = `${tmdbImageBase}${imagePath}`;
  return fetch(sourceUrl, {
    headers: {
      Accept: "image/*",
      "User-Agent": userAgent || "Otaku/0.2 (image proxy)"
    }
  });
}
