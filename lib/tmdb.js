...
function mapDetails(type, payload, aggregateCredits) {
  ...
  const seasons =
    type === "tv"
      ...
      : [];
  const creators = mapTitleCreators(type, payload);
...
function mapTitleCreators(type, payload) {
  const result = [];
  const seen = new Set();

  function pushCreator(role, person) {
    if (!person?.name) return;
    const key = `${role}:${person.id || person.name}`;
  ...
  if (type === "tv") {
    (payload.created_by || []).forEach((person) => pushCreator("Создатель", person));
  }

  const crew = payload.credits?.crew || [];
  const directorEntries = crew.filter((person) => person.job === "Director" || person.job === "Series Director");
  const writerEntries = crew.filter((person) =>
    ["Writer", "Screenplay", "Story", "Teleplay", "Series Composition"].includes(person.job)
  );

  directorEntries.forEach((person) => pushCreator("Режиссер", person));
  writerEntries.forEach((person) => pushCreator("Сценарист", person));

  const roleOrder = { "Создатель": 0, "Режиссер": 1, "Сценарист": 2 };
  return result.sort((a, b) => roleOrder[a.role] - roleOrder[b.role] || a.name.localeCompare(b.name, "ru-RU")).slice(0, 12);
}
...
export async function getPersonDetails({ id, language }) {
  ...
  const crewCredits = (payload.combined_credits?.crew || [])
    ...
    .map((item) => {
      ...
      return {
        ...
        title,
        year: (date || "").slice(0, 4) || null,
        character: item.job || item.department || null,
        popularity: item.popularity || 0,
        voteCount: item.vote_count || 0,
        poster: proxiedImage(item.poster_path, "w342")
      };
  ...
  for (const credit of combined) {
    ...
    if (!prev) {
      byTitleKey.set(key, credit);
      continue;
    }

    const prevScore = prev.popularity + prev.voteCount / 1000;
    const nextScore = credit.popularity + credit.voteCount / 1000;
    if (nextScore > prevScore) {
      byTitleKey.set(key, {
        ...credit,
        character: prev.character || credit.character
      });
      continue;
    }

    if (!prev.character && credit.character) {
      byTitleKey.set(key, { ...prev, character: credit.character });
    }
  }

  const credits = [...byTitleKey.values()]
...
async function fetchImageByPath(imagePath) {
  ...
  return fetch(sourceUrl, {
    headers: {
      Accept: "image/*",
      "User-Agent": "Otaku/0.2 (image proxy)"
    }
  });
}
